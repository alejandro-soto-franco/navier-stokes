...
regularity of Leray--Hopf weak solutions.  Classical approaches to the
regularity problem often require pointwise control of the vorticity
direction $\boldsymbol{\omega}/|\boldsymbol{\omega}|$, which is
ill-defined where $\boldsymbol{\omega}$ vanishes and demands regularity
beyond what the energy class provides.  The connection formulation sidesteps
this difficulty: by encoding the geometric content of the vorticity field
into the curvature of a well-defined connection on the divergence-free
bundle, one works entirely within the Leray--Hopf class
$L^{\infty}_{t}(L^{2}_{\sigma}) \cap L^{2}_{t}(H^{1}_{0})$ without
requiring pointwise estimates.  The programme then asks whether
